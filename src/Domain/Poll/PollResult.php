...
// то, что надо создать в результате
class PollResult
{
    /** @var int */
    private $id;
    ...
    public function __construct(int $id, Poll $poll, Employee $employee, Dish $dish, int $employeeFloor)
    {
        ...
        $this->dish = $dish;
        // 1. вот этот этаж надо проверить
    ...
    public function hasCorrectFloor(): bool
    {
        (($this->getEmployeeFloor() <= 1) && ($this->getEmployeeFloor() >= 6)) ? true : false;
        return false;
    }
}
